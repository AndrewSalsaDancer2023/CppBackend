...
namespace json_loader {
	...
	if(map_object.contains("bagCapacity"))
	{
		unsigned bag_capacity = map_object.at("bagCapacity").as_int64();
    ...
    void ParseMaps(const std::filesystem::path& json_path, model::Game& game)
    {
        ...
        if(object_to_read.contains("defaultBagCapacity"))
        	defaultBagCapacity = object_to_read.at("defaultBagCapacity").as_int64();
