...
namespace json_serializer {
   ...
   std::string GetPlayerInfoResponce(const std::vector<std::shared_ptr<model::Player>>& players_info)
   {
	  ...
	  //	for(const auto&[id, name] : players_info)
	    ...
	    for(auto& player : players)
	  	{
		    auto dog =  player->GetDog();
	  		...
	  		pos_ar.emplace_back(pos.first);
	  		pos_ar.emplace_back(pos.second);

	  		dog_object["pos"] = pos_ar;

	  		auto speed = dog->GetVelocity();
	  		speed_ar.emplace_back(speed.first);
			speed_ar.emplace_back(speed.second);
