...
public:
     explicit ApiHandler(model::Game& game, Strand& strand):game_{game}, strand_{strand}{
        ...
        {
        ticker_ = std::make_shared<Ticker>(strand_, std::chrono::milliseconds(game.GetTickPeriod()),[&game](std::chrono::milliseconds ticks)
        			{
        				//std::cout << "Called ticker before MoveDogs with count:" << ticks.count() << std::endl;
