...
//    	    {
  ...
  //  	    }
    ...
    StringResponse HandleGetGameState(http::verb method, std::string_view auth_type, const std::string& body, unsigned http_version, bool keep_alive)
    {
		...
		auto players =  game_.FindAllPlayersForAuthInfo(auth_token);
		auto resp = MakeStringResponse(http::status::ok, json_serializer::GetPlayersDogInfoResponce(players), http_version, keep_alive, ContentType::APPLICATION_JSON, "no-cache");
		return resp;
    }
    ...
    StringResponse HandlePlayerAction(http::verb method, std::string_view auth_type, const std::string& body, unsigned http_version, bool keep_alive)
    {
    	...
    	//std::cout << "Set dog speed" << std::endl;
    	...
    	auto map = game_.FindMap(model::Map::Id(session->GetMap()));
   		...
   		auto resp = MakeStringResponse(http::status::ok, "{}", http_version, keep_alive, ContentType::APPLICATION_JSON, "no-cache");
   		return resp;
    }
